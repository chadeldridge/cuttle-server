...
func getFile(file string) (MockFile, error) {
	...
	return MockFile{}, fmt.Errorf("MockTester %s: no such file or directory", file)
}
...
func MockTester(file string) error {
	...
	return fmt.Errorf("MockTester: permission denied")
}

func MockReader(file string) ([]byte, error) {
	if file == "" {
		return nil, fmt.Errorf("error: %s", FileNotFound)
	}
